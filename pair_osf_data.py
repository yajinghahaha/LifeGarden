...
# 联系文件（*_contact.json，新版代码把微信号与研究数据分开保存）：pid + session → 微信号；只在本地使用
CONTACT = {}
for _f in glob.glob(os.path.join(SRC, '**', '*_contact.json'), recursive=True):
    try:
        _j = json.load(open(_f, encoding='utf-8')); _j = _j[0] if isinstance(_j, list) else _j
        _w = (_j.get('wechat') or '').strip().lower().replace(' ', '')
        CONTACT[(_j.get('pid'), _j.get('session') or '')] = _w; CONTACT.setdefault((_j.get('pid'), ''), _w)
    except Exception: pass

def load_all():
    recs = []
    for f in glob.glob(os.path.join(SRC, '**', '*.json'), recursive=True):
        if f.endswith('_contact.json'): continue
        try:
        ...
        wx = CONTACT.get((j.get('pid'), j.get('session') or '')) or CONTACT.get((j.get('pid'), '')) or j.get('pair_wechat') or ((j.get('store') or {}).get('consent') or {}).get('contact_norm') or ((j.get('store') or {}).get('pair') or {}).get('wechat') or ''
...
for pair, g in by_pair.items():
    ...
    if d0:
        ...
        au = st.get('audio', {}); _d = au.get('duration') or 0; _u = au.get('unique_sec') or 0
        # 盲态：各条音轨时长不同，秒数会暴露组别；运行文件只给暴露比例（取到 0.05），秒数只进入解盲后的分析文件
        row.update({'audio_completed': au.get('completed'), 'audio_prop': (round(min(1.0, _u / _d) * 20) / 20) if _d else None, 'audio_pauses': au.get('pauses')})
...
for r in recs:
    ...
    if sf.get('flag_contact') or sf.get('flag_support'):
        safe_rows.append({'pair_code': r['pair'], 'wechat': (st.get('consent') or {}).get('contact', '') or CONTACT.get((r['json'].get('pid'), 'd0'), ''), 'd0_end': r['json'].get('end'),
