/* 生命花园 · 研究 1 网页版 · study.js
   会话由网址参数 session= 决定：d0（第一天）、d1（第二天）、fu（一周回访）。
...
(function () {
  ...
  // ---------- DataPipe ----------
  // DataPipe 新建实验默认开启数据校验：数据须为 JSON 数组、每行含 trial_type。这里包成一行，并在失败时记录错误、本机暂存、下次打开页面自动补传。
  const PENDING_KEY = 'lg_pending_uploads';
  const getPending = () => { try { return JSON.parse(localStorage.getItem(PENDING_KEY) || '[]'); } catch (e) { return []; } };
  const setPending = (a) => { try { localStorage.setItem(PENDING_KEY, JSON.stringify(a.slice(-10))); } catch (e) { } };
  async function postData(filename, dataStr) {
    const body = { experimentID: CFG.DATAPIPE_ID, filename, data: dataStr };
    let last = null;
    for (let i = 0; i < 3; i++) {
      try {
        const r = await fetch('https://pipe.jspsych.org/api/data/', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': '*/*' }, body: JSON.stringify(body) });
        if (r.ok) return { ok: true };
        let msg = ''; try { msg = await r.text(); } catch (e) { }
        last = 'HTTP ' + r.status + ' ' + msg.slice(0, 200);
        if (r.status >= 400 && r.status < 500 && !/exists/i.test(msg)) break;   // 校验类错误重试无用
      } catch (e) { last = String(e); }
      await new Promise(res => setTimeout(res, 1500));
    }
    console.warn('DataPipe 保存失败', filename, last);
    return { ok: false, error: last };
  }
  // 直接身份信息（微信号）与研究数据分开保存：研究数据文件中去掉微信号，另存一份只含连接码与微信号的联系文件
  function splitContact(obj) {
    let wx = null;
    const o = JSON.parse(JSON.stringify(obj));
    if (o.pair_wechat) { wx = o.pair_wechat; o.pair_wechat = null; }
    const st = o.store || {};
    if (st.consent) { wx = wx || st.consent.contact_norm || st.consent.contact || null; delete st.consent.contact; delete st.consent.contact_norm; st.consent.contact_given = !!wx; }
    if (st.pair && st.pair.wechat) { wx = wx || st.pair.wechat; delete st.pair.wechat; st.pair.wechat_given = true; }
    return { data: o, wx };
  }
  async function pipeSave(filename, objIn) {
    const sp = splitContact(objIn); const obj = sp.data;
    if (sp.wx) { const cf = filename.replace(/\.json$/, '_contact.json'); postData(cf, JSON.stringify([{ trial_type: 'lifegarden_contact', lg_file: cf, pid: obj.pid || PID, session: obj.session || SESSION, pair_code: obj.pair_code || PAIR || '', wechat: sp.wx, saved_at: nowISO() }])); }
    // 一行、全部是简单字段：trial_type 满足 DataPipe 默认校验；完整数据放在 payload_json 字符串里，避免嵌套结构触发校验或元数据错误
    const row = { trial_type: 'lifegarden_' + (obj.session || SESSION), lg_file: filename, pid: obj.pid || PID, session: obj.session || SESSION, platform: PLATFORM, pair_code: obj.pair_code || PAIR || '', version: CFG.VERSION, saved_at: nowISO(), payload_json: JSON.stringify(obj) };
    const dataStr = JSON.stringify([row]);
    const res = await postData(filename, dataStr);
    window.__lgLastSaveError = res.ok ? null : res.error;
    if (!res.ok) { const a = getPending(); a.push({ filename, dataStr, t: nowISO() }); setPending(a); }
    return res.ok;
  }
  async function flushPending() {
    const a = getPending(); if (!a.length) return; const keep = [];
    for (const it of a) { const r = await postData(it.filename.replace(/\.json$/, '_retry.json'), it.dataStr); if (!r.ok) keep.push(it); }
    setPending(keep);
  }
  flushPending();
  ...
  const slider0_10 = (name, label) => `<div class="lg-sl"><label>${label}</label><input type="range" name="${name}" min="0" max="10" value="0" step="1" oninput="this.nextElementSibling.textContent=this.value;this.style.setProperty('--fill',(this.value*10)+'%')"><output>0</output></div><div class="lg-sl-scale"><span>完全没有</span><span>非常强烈</span></div>`;
  ...
  function gardenTrial(tag, people) {
    ...
    const html = `<div class="lg-box lg-garden-box"><h3>今天的花园</h3><p>你手里有 <b>100 份养分</b>。拖动滑块，把它们分给下面五棵树——给得多的树会长得更高。合计正好 100；"现在的自己"至少 1 份。</p>
      <div class="lg-pool"><span>还剩</span><b id="lg-left">100</b><span>份养分</span><div class="lg-pool-bar"><div id="lg-pool-fill"></div></div></div>
      <div id="lg-garden">${order.map(i => `<div class="lg-tree-row"><div class="lg-tree" data-t="${keys[i]}"><span class="lg-tree-icon">🌱</span></div><div class="lg-tree-main"><div class="lg-tree-top"><span class="lg-name">${names[i]}</span><output>0</output></div><input type="range" data-k="${keys[i]}" min="0" max="100" value="0" step="1"></div></div>`).join('')}</div></div>`;
    return { type: jsPsychHtmlButtonResponse, stimulus: html, choices: ['提交分配'], data: { tag }, on_load: function () {
        ...
        const grow = (x) => { const v = +x.value; const t = document.querySelector(`.lg-tree[data-t="${x.dataset.k}"] .lg-tree-icon`); t.textContent = v === 0 ? '🌱' : (v < 20 ? '🌿' : '🌳'); t.style.transform = `scale(${0.8 + v / 70})`; x.style.setProperty('--fill', v + '%'); };
        const upd = () => { const sum = inputs.reduce((a, x) => a + (+x.value), 0); const left = 100 - sum; const el = document.getElementById('lg-left'); el.textContent = left; el.className = left < 0 ? 'lg-over' : (left === 0 ? 'lg-done' : ''); document.getElementById('lg-pool-fill').style.width = Math.max(0, Math.min(100, left)) + '%'; const S = +inputs.find(x => x.dataset.k === 'S').value; b.disabled = !(sum === 100 && S >= 1); };
        inputs.forEach(x => { x.addEventListener('input', () => { x.parentElement.querySelector('output').textContent = x.value; grow(x); trace.push([Math.round(performance.now() - t0), x.dataset.k, +x.value]); upd(); }); });
  ...
  function finishTrial(kind, extraFn) {
    return { type: jsPsychHtmlButtonResponse, choices: [], trial_duration: 999999, stimulus: '<div class="lg-box"><h3>正在保存…</h3><p id="lg-save">请不要关闭页面。</p></div>', data: { tag: 'finish' }, on_load: async function () {
        ...
        const saved = (ok ? '<b>已保存。</b>' : `<b>保存遇到问题</b>，请截图此页发给研究者。<br><span class="lg-muted">${(window.__lgLastSaveError || '').replace(/</g, '&lt;')}</span>`) + `<br><span class="lg-muted" style="font-size:11px">${CFG.VERSION}</span>`;
  ...
  function buildD0() {
    ...
    // 同意
    T.push({ type: jsPsychSurveyHtmlForm, preamble: `${(() => { try { return localStorage.getItem('lg_d0_done') ? '<div class="lg-card">这台手机之前已经参加过本研究的第一天。同一个人只能参加一次；如果你确定是第一次参加，可以继续。</div>' : ''; } catch (e) { return ''; } })()}<div class="lg-box"><h2>参加之前，请先看这几条</h2>
      <p><b>这是什么。</b>一个关于"引导练习"的研究。今天约 30 分钟：几份问卷、一个"分养分"的小任务、戴耳机听一段约 11 分钟的引导练习。第二天约 10 分钟问卷（从明天起一周内任何时候）。一周后有一次约 3 分钟的回访，参不参加你定。</p>
      ...
      <p><b>退出。</b>随时可以退出，不用说理由。报酬按天发放：做完哪一天、拿到那一天的完成码，就有那一天的报酬；已经得到的报酬不会收回。练习中间不想听了可以退出练习，继续做后面的题，同样算完成。</p>
      <p><b>录音。</b>练习后会请你说一段感受。可以录音，也可以打字，也可以跳过。录音只有研究人员会听，会转成文字并去掉能认出你的信息，原始录音在研究结束后一年内删除。</p>
      <p><b>报酬。</b>今天做完一份，第二天做完一份，一周后回访再一份。第二天还有一小笔钱，你自己决定现在领、一个月后领，还是捐掉。钱由平台或研究者的微信发给你，数额见任务说明。</p>
      <p><b>微信。</b>请留下你的微信号。研究者会加你，用来提醒你做第二天的任务、发放报酬，以及你有需要时联系你，不做别的用途。微信号和你的作答分开保存。</p>
      <p><b>你的信息。</b>我们会问出生年月、性别、职业、工作年限。数据只用编号，不会出现你的名字。今天结束时你会得到一个 8 位码，请截图，第二天和回访都要用。</p>
      <p><b>数据存在哪里。</b>作答经加密传输，保存在研究者私人的、只有研究人员能访问的云端文件夹（Google Drive，服务器在中国大陆以外）。</p>
      <p><b>文字分析。</b>去掉能认出你的信息之后，你写的文字可能由人工智能语言模型辅助整理，下面单独询问你是否同意，不同意也完全不影响参加。</p></div>`,
      html: `<div class="lg-box lg-form"><label class="lg-radio"><input type="checkbox" name="c1" required> 我看懂了，自愿参加</label><label class="lg-radio"><input type="checkbox" name="c2" required> 我知道随时可以退出；报酬按完成的天数发放，已得到的报酬不会收回</label><label class="lg-radio"><input type="checkbox" name="c3" required> 我同意研究者通过微信联系我</label>
      <div class="lg-q"><div>录音：</div>${radio('rec', ['可以录音（也可以打字或跳过）', '只打字，不录音'])}</div>
      <div class="lg-q"><div>文字分析（可选）：</div>${radio('ai_ok', [{ t: '同意：去掉可识别信息后，我的文字可由人工智能模型辅助整理', v: 1 }, { t: '不同意：只由研究人员阅读', v: 0 }])}</div>
    ...
    T.push(form('几个筛查问题', `
      ...
      <div class="lg-q"><div>你的听力是否正常（能听清手机播放的语音）？</div>${radio('hear', [{ t: '正常', v: 0 }, { t: '有听力问题', v: 1 }])}</div>
      <div class="lg-q"><div>今天听练习时你会用：</div>${radio('headphones', [{ t: '耳机', v: 1 }, { t: '没有耳机，在安静的地方用外放', v: 0 }])}</div>
      <div class="lg-q"><div>之前是否参加过"生命花园"这项研究（听过这段引导练习）？</div>${radio('prior', [{ t: '否', v: 0 }, { t: '是', v: 1 }])}</div>
    ...
    T.push(form('基本信息', `
      ...
      <div class="lg-q"><div>你觉得自己大概能活到多少岁？（填一个数字）</div><input type="number" name="sle" min="1" max="150" required></div>
      <div class="lg-q"><div>你预计自己大约几岁退休？（填一个数字；还没想过就填一个大概的年龄）</div><input type="number" name="ret_age" min="18" max="120" required></div>
      ...
      <div class="lg-q"><div>过去四周做过几天？</div><input type="number" name="med_days" min="0" max="28" required></div>`, 'baseline', { on_load: () => { const f = document.querySelector('#jspsych-survey-html-form'); if (f) f.addEventListener('submit', (e) => { const by = +f.querySelector('[name=birth_year]').value, fj = +f.querySelector('[name=first_job_year]').value, sle = +f.querySelector('[name=sle]').value, ra = +f.querySelector('[name=ret_age]').value, age = new Date().getFullYear() - by; const msg = (fj < by + 14 || fj > new Date().getFullYear()) ? '"第一次全职工作年份"看起来不对：请填四位年份，且应晚于出生年份 14 年以上。' : (sle <= age ? '"能活到多少岁"应大于你现在的年龄。' : (ra <= age ? '"预计退休年龄"应大于你现在的年龄；已退休请填现在的年龄加 1。' : '')); if (msg) { e.preventDefault(); e.stopImmediatePropagation(); alert(msg); } }, true); }, on_finish: (d) => { const r = d.response; const tenure = (new Date().getFullYear()) - (+r.first_job_year); r.tenure_years = tenure; r.stratum = (tenure <= 5 ? 'T1' : (tenure <= 15 ? 'T2' : 'T3')) + '_' + r.sex; log('baseline', r); } }));
    // 死亡认知、目标与底线、沉没成本（随机化前）
    T.push(form('几个关于自己的问题', `
      <div class="lg-q"><div>你第一次意识到"人终有一死"，大约几岁？</div><input type="number" name="dc_age" min="1" max="100" required></div>
    ...
    // 随机化
    T.push({ type: jsPsychCallFunction, async: true, func: async (done) => { const a = await assignCell(store.baseline.stratum); log('assignment', { cell: a.cell, cond: a.cond, source: a.source, stratum: store.baseline.stratum }); try { localStorage.setItem('lg_d0_done', nowISO()); } catch (e) { } window.__cell = a.cell; try { const pre = new Audio(); pre.preload = 'auto'; pre.src = CFG.AUDIO[a.cell]; window.__lgPre = pre; } catch (e) { } try { localStorage.setItem(LS('cell'), a.cell); } catch (e) { } done(); } });
    // 音频前提示
    ...
    // 播放器（不可拖动）
    T.push({ timeline: [{ type: jsPsychHtmlButtonResponse, stimulus: () => H(`<h3>引导练习</h3><p id="lg-audio-msg">点击下方按钮开始播放，播放完成后会出现"继续"。</p><audio id="lg-audio" src="${CFG.AUDIO[window.__cell]}" preload="auto"></audio><div class="lg-breath"><div class="lg-breath-ring"></div><div class="lg-breath-core"><span id="lg-state">未开始</span></div></div><div class="lg-player"><button type="button" id="lg-play">播放</button> <button type="button" id="lg-pause">暂停</button></div><p class="lg-muted">没有进度条，不能拖动；可以暂停，也可以退出。</p><p id="lg-stall" style="display:none;text-align:center"><button type="button" id="lg-reload">声音卡住了？点这里重新加载（从刚才的位置继续）</button></p>`), choices: ['继续', '退出这次练习'], data: { tag: 'audio' }, on_load: () => {
        ...
        document.getElementById('lg-play').onclick = () => { a.play(); plays++; document.getElementById('lg-state').textContent = '正在播放'; document.querySelector('.lg-breath').classList.add('on'); ev('audio_play'); };
        document.getElementById('lg-pause').onclick = () => { a.pause(); pauses++; document.getElementById('lg-state').textContent = '已暂停'; document.querySelector('.lg-breath').classList.remove('on'); ev('audio_pause'); };
        a.addEventListener('timeupdate', () => { const c = Math.floor(a.currentTime); if (c > lastT + 2) { a.currentTime = lastT; } else { secs.add(c); lastT = c; } });
        a.addEventListener('ended', () => { btns[0].disabled = false; document.querySelector('.lg-breath').classList.remove('on'); document.getElementById('lg-state').textContent = '播放完成'; document.getElementById('lg-audio-msg').textContent = '练习结束，请点"继续"。'; ev('audio_ended'); });
        let reloads = 0, stallTimer = null; const stallBox = document.getElementById('lg-stall');
        const showStall = () => { if (!a.ended) stallBox.style.display = 'block'; };
        const armStall = () => { clearTimeout(stallTimer); stallTimer = setTimeout(showStall, 6000); };
        a.addEventListener('waiting', () => { if (!a.paused) armStall(); });
        a.addEventListener('stalled', () => { if (!a.paused) armStall(); });
        a.addEventListener('error', showStall);
        a.addEventListener('playing', () => { clearTimeout(stallTimer); stallBox.style.display = 'none'; });
        a.addEventListener('timeupdate', () => { clearTimeout(stallTimer); });
        document.getElementById('lg-reload').onclick = () => { const pos = lastT; reloads++; ev('audio_reload', { pos }); stallBox.style.display = 'none'; document.getElementById('lg-state').textContent = '正在重新加载…'; a.load(); a.addEventListener('loadedmetadata', function once() { a.removeEventListener('loadedmetadata', once); try { a.currentTime = pos; } catch (e) { } a.play().then(() => { document.getElementById('lg-state').textContent = '正在播放'; document.querySelector('.lg-breath').classList.add('on'); }).catch(() => { document.getElementById('lg-state').textContent = '请点"播放"继续'; }); }); };
        document.addEventListener('visibilitychange', () => ev('visibility', { hidden: document.hidden }));
        window.__lgAudio = () => ({ unique_sec: secs.size, duration: a.duration || null, plays, pauses, reloads, wall_sec: Math.round((Date.now() - t0) / 1000) });
    ...
    // 结束
    T.push(btn(`<h3>今天的任务完成了</h3><p><b>第二天的任务（约 10 分钟）从明天起、一周之内任何时候都可以做，越早越好——请睡一觉之后再来。</b>我们会通过你留下的方式提醒你；请从平台发给你的链接进入。</p><p>刚才的内容是一种想象练习，不是对你生活的判断，也不是对未来的预言。</p><div class="lg-card">${CFG.RESOURCE_CARD}</div>`, { choices: ['保存并结束'] }));
  ...
  function codeEntryTrials(kind, enforceGap) {
    ...
    const early = { timeline: [{ type: jsPsychHtmlButtonResponse, stimulus: () => H(`<h3>还没到时间</h3><p>第二天的任务需要在第一天完成至少 ${CFG.MIN_GAP_HOURS} 小时之后进行——请睡一觉之后再来（一周之内任何时候都可以）。你的连接码仍然有效。</p>`), choices: [], trial_duration: 999999, data: { tag: 'pair_early' } }], conditional_function: () => enforceGap && store.pair.gap_hours !== null && store.pair.gap_hours < CFG.MIN_GAP_HOURS };
    return [loop, early];
  }
  ...
  function buildD1() {
    ...
    T.push(form('最后几个问题', `
      ...
      <div class="lg-q"><div>一周后我们会邀请你做一次约 3 分钟的回访，愿意的话我们会通过你留下的方式联系你。</div>${radio('fu_willing', [{ t: '愿意', v: 1 }, { t: '再说', v: 0 }])}</div>`, 'closing'));
  ...
  function buildFU() {
    ...
    T.push(form('几个问题', `${likert('p_scar1', '此刻，我觉得自己剩下的时间不多了', '完全不符合', '完全符合')}${likert('p_scar2', '此刻，我觉得时间很紧、来不及做想做的事', '完全不符合', '完全符合')}${likert('p_unpred', '此刻，我觉得人生中的事情很难预料', '完全不符合', '完全符合')}${likert('p_inev', '此刻，我清楚地意识到生命终有尽头', '完全不符合', '完全符合')}
      <div class="lg-q"><div>第一天和第二天写下的三件事，做了多少？</div>${radio('plan_done', ['都没做', '做了一件', '做了两件', '三件都做了', '记不清写了什么'])}</div>
      <div class="lg-q"><div>这一周多里，有没有自己做过类似的练习？</div>${radio('self_practice', ['没有', '一两次', '经常'])}</div>
      <div class="lg-q"><div>这一周多里，生活中有没有发生重大事件（比如换工作、搬家、亲人生病等）？</div>${radio('life_event', [{ t: '没有', v: 0 }, { t: '有', v: 1 }])}<input type="text" name="life_event_txt" placeholder="如果有，可简单写一下（可不填）"></div>`, 'fu'));
