...
(function () {
  ...
  function buildD0() {
    ...
    // 播放器（不可拖动）
    T.push({ timeline: [{ type: jsPsychHtmlButtonResponse, stimulus: () => H(`<h3>引导练习</h3><p id="lg-audio-msg">点击下方按钮开始播放，播放完成后会出现"继续"。</p><audio id="lg-audio" src="${CFG.AUDIO[window.__cell]}" preload="auto"></audio><div class="lg-breath"><div class="lg-breath-ring"></div><div class="lg-breath-core"><span id="lg-state">未开始</span></div></div><div class="lg-player"><button type="button" id="lg-play">播放</button> <button type="button" id="lg-pause">暂停</button></div><p class="lg-muted">没有进度条，不能拖动；可以暂停，也可以退出。</p>`), choices: ['继续', '退出这次练习'], data: { tag: 'audio' }, on_load: () => {
